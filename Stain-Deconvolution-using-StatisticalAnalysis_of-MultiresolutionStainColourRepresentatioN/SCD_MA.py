...
def SCD_MA(img=None):
   ...
   G = img[:, :, 1]
   R = img[:, :, 0]
   RGB = np.array([R.reshape(( h * w)), G.reshape(( h * w)), B.reshape(( h * w))])  # LT: remove dim 1
   ...
   for i in range(0, Level):
      ...
      NumEle = LL_1.size
      Kor = np.array([kurtosis(LL_1.flatten()) - 3,
                      kurtosis(LH_1.flatten()) - 3,
                      kurtosis(HL_1.flatten()) - 3,
                      kurtosis(HH_1.flatten()) - 3])
      Kor = np.abs(Kor)
      z = 0
      for s in range(Statcount, Statcount + 3):
         StatMesur[0, s] = Kor[0]
         StatMesur[1, s] = i
         StatMesur[2, s] = z
         z = z + 1
   ...
   G = Coff[:, :, 1]
   R = Coff[:, :, 0]
   Coff = np.array([np.reshape(R, ( r * c)),  #LT: remove dim of 1
                     np.reshape(G, ( r * c)),
                     np.reshape(B, ( r * c))])
   ...
   for i in range(1, NumBands):
      Coff = Bands[int(StatMesur[1, i])][int(StatMesur[2, i])]            #Bands{level,band}
      r, c, _ = Coff.shape
      B = Coff[:, :, 2]
      G = Coff[:, :, 1]
      R = Coff[:, :, 0]
   ...
   fastica = FastICA(n_components=3,max_iter=5000)
   
   
   
   A = fastica.fit_transform(zca)
...
SCD_MA(np.array(img, dtype=np.float32)) 
